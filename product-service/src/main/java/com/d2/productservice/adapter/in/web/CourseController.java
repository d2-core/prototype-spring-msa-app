...
import com.d2.productservice.model.request.CourseUpsertFileRequest;
...
public class CourseController {
	...
	public API<Course> registerCourse(
		...
		@Valid @ModelAttribute CourseUpsertFileRequest fileRequest,
		@Valid @RequestPart("request") CourseUpsertRequest bodyRequest
	) {
		if (!adminUserAuth.getAdminUserId().equals(AuthConstant.NOT_EXIST)) {
			Course course = courseUseCase.upsertCourse(null, bodyRequest.getTeacherId(),
				fileRequest.getThumbnailImageFiles(), bodyRequest.getCourseCategoryId(), bodyRequest.getTitle(),
				bodyRequest.getSubTitle(), bodyRequest.getDescriptionWithMarkdown(), bodyRequest.getCourseLevelId(),
				bodyRequest.getTags(), bodyRequest.getPrice());
	...
	public API<Course> updateCourse(
		...
		if (!adminUserAuth.getAdminUserId().equals(AuthConstant.NOT_EXIST)) {
			Course course = courseUseCase.upsertCourse(courseId, bodyRequest.getTeacherId(),
				fileRequest.getThumbnailImageFiles(), bodyRequest.getCourseCategoryId(), bodyRequest.getTitle(),
				bodyRequest.getSubTitle(), bodyRequest.getDescriptionWithMarkdown(), bodyRequest.getCourseLevelId(),
				bodyRequest.getTags(), bodyRequest.getPrice());
			return API.OK(course);
	...
	public API<Object> publishCourse(
		...
		if (!adminUserAuth.getAdminUserId().equals(AuthConstant.NOT_EXIST)) {
			...
			} else if (request.getPublishState().equals(CoursePublishState.PRIVATE)) {
	...
	@GetMapping("v1/courses/{courseId}/lectures")
	public API<List<Lecture>> getCourseLectureList(
		@AdminUserAuthInjection AdminUserAuth adminUserAuth,
		@UserAuthInjection UserAuth userAuth,
		@PathVariable Long courseId
	) {
		if (!adminUserAuth.getAdminUserId().equals(AuthConstant.NOT_EXIST)) {
			List<Course> course = courseUseCase.getCourseList();
			return API.OK(courseUseCase.getCourseLectureList(courseId));
		}
